...
QtIOCompressorPrivate::QtIOCompressorPrivate(
    QtIOCompressor *q_ptr, QIODevice *device, int compressionLevel, int bufferSize)
...
bool QtIOCompressorPrivate::writeBytes(ZlibByte *buffer, ZlibSize outputSize)
{
    ...
    do {
        ...
        if (bytesWritten == -1) {
            q->setErrorString(
                QT_TRANSLATE_NOOP("QtIOCompressor", "Error writing to underlying device: ") + device->errorString());
            return false;
        }
...
bool QtIOCompressor::open(OpenMode mode)
{
    ...
    if (d->device->isOpen()) {
        ...
        if (d->device->open(mode) == false) {
            setErrorString(
                QT_TRANSLATE_NOOP("QtIOCompressor", "Error opening underlying device: ") + d->device->errorString());
            return false;
        }
    }
        ...
        if (d->streamFormat == QtIOCompressor::ZlibFormat) {
            ...
            if (checkGzipSupport(zlibVersion()) == false) {
                setErrorString(QT_TRANSLATE_NOOP(
                    "QtIOCompressor::open", "The gzip format not supported in this version of zlib."));
                return false;
            }
