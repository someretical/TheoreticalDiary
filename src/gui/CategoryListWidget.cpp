...
CategoryListWidget::CategoryListWidget(QWidget *parent)
    : QWidget(parent), m_itemDelegate(nullptr), m_ui(new Ui::CategoryListWidget())
{
    m_ui->setupUi(this);
...
auto CategoryListWidget::sizeHint() const -> QSize
{
...
auto CategoryListWidget::minimumSizeHint() const -> QSize
{
    return {
        m_itemDelegate->minWidth() + m_ui->categoryList->frameWidth() * 2, m_ui->categoryList->sizeHintForRow(0) * 2};
}

auto CategoryListWidget::addCategory(const QString &labelText, const QIcon &icon) -> int
{
...
auto CategoryListWidget::currentCategory() -> int
{
...
auto CategoryListWidget::isCategoryHidden(int index) -> bool
{
...
void CategoryListWidget::updateCategoryScrollButtons()
{
    ...
    m_ui->scrollDown->setEnabled(
        m_ui->categoryList->verticalScrollBar()->value() != m_ui->categoryList->verticalScrollBar()->maximum());
...
CategoryListWidgetDelegate::CategoryListWidgetDelegate(QListWidget *parent)
    : QStyledItemDelegate(parent), m_listWidget(parent), m_size(96, 96)
{
...
auto CategoryListWidgetDelegate::minWidth() const -> int
{
...
auto CategoryListWidgetDelegate::sizeHint(const QStyleOptionViewItem &option, const QModelIndex &index) const -> QSize
{
    Q_UNUSED(option)
    Q_UNUSED(index)
