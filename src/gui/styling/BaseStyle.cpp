...
{
    QColor dark;
    dark.setHsv(option->palette.button().color().hue(), qMin<int>(255, (option->palette.button().color().saturation())),
...
QIcon BaseStyle::standardIcon(StandardPixmap sp, const QStyleOption *opt, const QWidget *widget) const
{
    ...
    //        return icons()->icon(
    //            QString("edit-clear-locationbar-").append((opt->direction ==
    //            Qt::LeftToRight) ? "rtl" : "ltr"));
    //    default:
    //    }
    ...
    ComplexControl control, const QStyleOptionComplex *option, QPainter *painter, const QWidget *widget) const
{
    ...
    case CC_ScrollBar: {
        ...
        // Width and height should be reduced by 2 * padding, but somehow padding is
        // enough.
