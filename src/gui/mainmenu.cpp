...
MainMenu::MainMenu(QWidget *parent) : QWidget(parent), ui(new Ui::MainMenu)
{
    ...
    connect(
        enter, &QShortcut::activated, this, [&]() { QMetaObject::invokeMethod(ui->decrypt_button, "clicked"); },
        Qt::QueuedConnection);
    ...
    connect(TheoreticalDiary::instance(), &TheoreticalDiary::apply_theme, this, &MainMenu::apply_theme,
        Qt::QueuedConnection);
    apply_theme();
}
...
bool MainMenu::get_diary_contents()
{
    ...
    std::ifstream first(TheoreticalDiary::instance()->data_location().toStdString() + "/diary.dat");
    ...
    if (!first.fail()) {
        ...
        std::ifstream second(TheoreticalDiary::instance()->data_location().toStdString() + "/diary.dat.bak");
...
void MainMenu::decrypt_diary()
{
    ...
    TheoreticalDiary::instance()->encryptor->set_decrypt_iv(str->substr(0, IV_SIZE));
    str->erase(0, IV_SIZE);
    ...
    connect(&TheoreticalDiary::instance()->worker_thread, &QThread::finished, worker, &QObject::deleteLater,
        Qt::QueuedConnection);
    connect(worker, &HashWorker::done, this, &MainMenu::decrypt_diary_cb, Qt::QueuedConnection);
...
void MainMenu::decrypt_diary_cb(bool const do_decrypt)
{
    ...
    if (Zipper::unzip(do_decrypt ? decrypted : *TheoreticalDiary::instance()->encryptor->encrypted_str, decompressed) &&
