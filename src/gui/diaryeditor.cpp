...
void DiaryEditor::render_month(const QDate &date, const std::optional<td::YearMap::iterator> &opt)
{
    ...
    for (int i = current_month_offset; i < 7; ++i, ++days_added) {
        ...
        if (opt) {
            ...
            if (entry_iter != monthmap.end()) {
                ...
                const td::CalendarButtonData d{std::optional(i - current_month_offset + 1), std::optional(this),
    ...
    for (int i = days_added; i < date.daysInMonth(); ++i, ++current_row_length) {
        ...
        if (opt) {
            ...
            if (entry_iter != monthmap.end()) {
                const auto &[important, rating, dummy, d2] = entry_iter->second;
                const td::CalendarButtonData d{std::optional(i + 1), std::optional(this), std::optional(important),
        ...
        const td::CalendarButtonData d{std::optional(i + 1), std::optional(this), std::optional(false),
...
bool DiaryEditor::confirm_switch()
{
    ...
    confirm.setInformativeText("Are you sure you want to switch dates without saving?");
...
void DiaryEditor::change_month(const QDate &date, const bool suppress_confirm)
{
    ...
    const QDate first_day(date.year(), date.month(), 1);
...
void DiaryEditor::render_day(const td::CalendarButtonData &d, const bool set_info_pane)
{
    ...
    const auto &button = qobject_cast<CalendarButton *>(ui->dates->itemAtPosition(y, x)->widget());
    button->re_render(d);

    if (set_info_pane) {
        const auto &current_date = QDate(ui->year_edit->date().year(), ui->month_dropdown->currentIndex() + 1, *d.day);
        const auto &opt = TheoreticalDiary::instance()->diary_holder->get_entry(current_date);
...
void DiaryEditor::date_clicked(const int day)
{
    ...
    const td::CalendarButtonData old{
        std::optional(last_selected_day), std::nullopt, std::nullopt, std::nullopt, std::optional(false)};
    const td::CalendarButtonData n{std::optional(day), std::nullopt, std::nullopt, std::nullopt, std::optional(true)};
...
void DiaryEditor::update_day(const bool suppress_message)
{
    ...
    const td::Entry e{ui->special_box->isChecked(), static_cast<td::Rating>(ui->rating_dropdown->currentIndex()),
    ...
    const auto &res = qobject_cast<MainWindow *>(
...
void DiaryEditor::delete_day()
{
    ...
    const td::CalendarButtonData d{std::optional(last_selected_day), std::nullopt, std::optional(false),
